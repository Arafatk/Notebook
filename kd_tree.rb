...
class Kdtree
  ...
  def make_tree(points,dim,i)
    ...
    if left.size>1
      root.left = make_tree(left,dim,(i + 1) % dim)
    end
    if right.size==0
      root.right=nil
    end
    ...
    if right.size>1
       root.right = make_tree(right,dim,(i + 1) % dim)
     end
  ...
  def near(nearest,size)
    rbtree=MultiRBTree[]
    a=nearest(root,nearest,0,@dime,nil,1000000,0,rbtree,size)
  end
...
b=Kdtree::Node.new([23,4,5])
a=Kdtree::Node.new([23,4,5,12],b)
#print a.point,"\n",a.left.point,"\n\n\n"
c=Kdtree.new(   [[2, 3],[5, 4], [9, 6], [3, 7],
         [4, 7], [8, 1], [7, 2], [6, 2],
         [14, 17],[2, 2],[6, 6],[11, 2],
         [7, 4],[9, 1],[5, 1],[7, 1]],    2)
a=c.near([9,2],9)
